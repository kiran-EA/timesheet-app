    ...
    <div className="flex flex-col h-full" style={{ background: t.pageBg }}>
      <div className="flex items-center justify-between px-8 h-[70px] flex-shrink-0"
        ...
        <div>
          ...
          <p className="text-sm" style={{ color: t.textMuted }}>Fetch Google Calendar events and log them as timesheet entries</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-6">
        ...
        {fetched && visible.length > 0 && (
          <div className="rounded-xl overflow-hidden shadow-sm" style={{ background: t.cardBg, border: t.border }}>
            <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: t.border }}>
              <h3 className="text-base font-semibold" style={{ color: t.text }}>
                Events -{' '}
                {new Date(date + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                <span className="ml-2 text-sm font-normal" style={{ color: t.textMuted }}>({visible.length})</span>
              </h3>
            ...
            {pendingCount > 0 && (
              <div className="px-6 py-4 flex justify-end" style={{ borderTop: t.border }}>
                <button onClick={submitAll} disabled={saving}
                  className="px-6 py-2.5 rounded-lg text-white font-semibold text-sm hover:opacity-90 disabled:opacity-50 transition-opacity"
                  style={{ background: 'linear-gradient(135deg,#10b981,#059669)' }}>
                  {saving ? 'Saving...' : `Submit All (${pendingCount})`}
                </button>
              </div>
